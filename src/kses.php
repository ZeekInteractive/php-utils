 ...
 * - Return WP version of function if present.
 */
...
function get_allowed_entity_names() {
	...
	$allowed_post_tags = array_map( '\Zeek\PHP_Util\_wp_add_global_attributes', $allowed_post_tags );
...
function _wp_add_global_attributes( $value ) {
	if ( function_exists( '\_wp_add_global_attributes' ) ) {
		return \_wp_add_global_attributes( $value );
	}
...
function wp_kses_split( $string, $allowed_html, $allowed_protocols ) {
	...
	return preg_replace_callback( '%(<!--.*?(-->|$))|(<[^>]*(>|$)|>)%', '\Zeek\PHP_Util\_wp_kses_split_callback', $string );
}
...
function _wp_kses_split_callback( $match ) {
	...
	return wp_kses_split2( $match[0], null, null );
}
...
function wp_kses_attr_check( &$name, &$value, &$whole, $vless, $element, $allowed_html ) {
	if ( function_exists( '\wp_kses_attr_check' ) ) {
		return \wp_kses_attr_check( $name, $value, $whole, $vless, $element, $allowed_html );
	}
...
function wp_kses_allowed_html( $context = '' ) {
	...
	switch ( $context ) {
		...
		case 'entities':
			return get_allowed_entity_names();
...
function wp_kses_normalize_entities( $string ) {
	...
	$string = preg_replace_callback( '/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', '\Zeek\PHP_Util\wp_kses_named_entities', $string );
	$string = preg_replace_callback( '/&amp;#(0*[0-9]{1,7});/', '\Zeek\PHP_Util\wp_kses_normalize_entities2', $string );
	$string = preg_replace_callback( '/&amp;#[Xx](0*[0-9A-Fa-f]{1,6});/', '\Zeek\PHP_Util\wp_kses_normalize_entities3', $string );

	return $string;
}

/**
 * Helper functions for formatting. Taken from WordPress for cross-compatibility.
 */
...
function wp_kses_named_entities( $matches ) {
	...
	return ( ! in_array( $i, get_allowed_entity_names() ) ) ? "&amp;$i;" : "&$i;";
}
...
function wp_kses_decode_entities( $string ) {
	...
	$string = preg_replace_callback( '/&#([0-9]+);/', '\Zeek\PHP_Util\_wp_kses_decode_entities_chr', $string );
	$string = preg_replace_callback( '/&#[Xx]([0-9A-Fa-f]+);/', '\Zeek\PHP_Util\_wp_kses_decode_entities_chr_hexdec', $string );
...
function _wp_kses_decode_entities_chr( $match ) {
	if ( function_exists( '\_wp_kses_decode_entities_chr' ) ) {
		return \_wp_kses_decode_entities_chr( $match );
	}
...
function _wp_kses_decode_entities_chr_hexdec( $match ) {
	if ( function_exists( '\_wp_kses_decode_entities_chr_hexdec' ) ) {
		return \_wp_kses_decode_entities_chr_hexdec( $match );
	}
